...
def wav_to_mp3(in_file, out_file, rate=44100, channels=2, bits=192):
    bashCommand = "ffmpeg -i {in_file} -vn -ar {rate} -ac {channels} -ab {bits} -f mp3 {out_file}".format(
...
def mp3_to_wav(in_file, out_file):
    bashCommand = "ffmpeg -i {in_file} {out_file}".format(in_file=in_file, out_file=out_file)
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    #output = process.communicate()[0]